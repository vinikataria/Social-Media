...
function Articles() {
  ...
  useEffect(() => {
    ...
    const emailToSend = isAdminUser ? "" : Loginemail || ""; // Admin gets all articles (empty email)

    const url = "https://localhost:7160/api/Article/ArticleList";
    const data = {
      ID: 0,
      Content: "",
      Title: "",
            ...
            {data.map((data, index) => {
              ...
              return (
                <tr key={index}>
                  ...
                  <td>{data.email}</td>
                  <td>{data.image}</td>
                  <td>{data.isApproved === 1 ? "Approved" : "Pending"}</td>
                  <td>
                    {isAdmin && (
                      <button
